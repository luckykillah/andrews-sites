<?php
/*
  Plugin Name: Acronyms 2
  Description: A fork of Joel Pan's original Acronyms plugin (no longer actively maintained), this is a plugin to wrap acronyms in posts and comments with appropriate <samp>&lt;acronym&gt;</samp> or <samp>&lt;abbr&gt;</samp> markup. Allows users to manage lists of acronyms and abbreviations through admin interface. Based on Joel Bennett's Acronym Replacer plugin (http://www.huddledmasses.org/) and Joel Pan's NP_Acronym plugin for Nucleus CMS.
  Version: 2.0.1
  Author: Chris Cox
  Author URI: http://www.renaissance-design.net

  Copyright 20011  Chris Cox
...
class Acronyms {
	...
	add_option('acronym_meta', 1);
	Acronyms::theme_html5_check();
    }
	...
	if ('acronyms.php' == substr($parent_file, -12)) {
		...
		case 'add-acronym':
		    ...
		    if (Acronyms::update($acronym, $fulltext))
			$message = 1;
		    else
			$message = 4;
	...
	if (0 == $n)
	    ...
	    $t = ceil(Acronyms::count_acronyms($s) / $n); // Total number of pages, rounded up to nearest integer
	?>
	<div class="wrap">
	...
	wp_nonce_field($nonce_action); ?>
		...
		<input name="acronym" id="acronym" type="text" value="<?php echo attribute_escape($acronym); ?>" size="20" <?php if ('edit-acronym' == $action)
	...
	<?php if ('edit-acronym' == $action)
	    echo '<a accesskey="c" title="' . __('Cancel') . '" class="cancel button-secondary alignright" href="tools.php?page=' . Acronyms::get_parent_url() . '">' . __('Cancel') . '</a>' ?>
...
register_uninstall_hook( __FILE__, array( 'Acronyms', 'uninstall' ) );
...
if (1 == get_option('acronym_meta'))	
    add_filter('the_meta', array('Acronyms', 'acronym_replace'));

add_action('admin_menu', array('Acronyms', 'add_pages'));

add_action('switch_theme', array('Acronyms', 'theme_html5_check'));
?>